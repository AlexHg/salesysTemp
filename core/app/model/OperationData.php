...
class OperationData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (product_id,q,operation_type_id,sell_id,created_at,local_id) ";
		$sql .= "value (\"$this->product_id\",\"$this->q\",$this->operation_type_id,$this->sell_id,$this->created_at,$this->local_id)";
...
// partiendo de que ya tenemos creado un objecto OperationData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set product_id=\"$this->product_id\",q=\"$this->q\",local_id=$this->local_id where id=$this->id";
		Executor::doit($sql);
	}
	...
	public static function getByLocalId($id){
		$sql = "select * from ".self::$tablename." where local_id=$id";
		$query = Executor::doit($sql);
		return Model::one($query[0],new OperationData());
	}
